...
route and the "England — 30k" sub-tab produce demographically EQUIVALENT
agents — census-representative on every marginal and internally correlated by
the same eight-pass cascade (app.py now calls apply_all_correlations on the
preset route). The two routes are not bit-for-bit identical: the sub-tab adds
local-authority-level geography (occupation drawn from each LA's TS063 profile,
region from a population-weighted place draw) that the flat preset configs do
not. See the NOTE lower in this file for the details.
...
# NOTE — base-sampler route now applies the full correlation cascade:
# app.py's base sampler draws each field INDEPENDENTLY, but after sampling it
# calls uk_england_population.apply_all_correlations() on every England skeleton
# (any preset that supplies economic_activity). That single orchestrator runs
# all eight passes — age, education→occupation→grade→income, health↔disability↔
# activity, tenure, age→marital→household, ethnicity→religion+urban/rural,
# education→attitudes, and the WVS value covariances. So preset-route agents are
# now internally correlated too: a retired agent's occupation is set to N/A, a
# highly religious agent's belief/ethics items are nudged together, teenage
# widows are removed, ethnicity drives religion, and so on.
#
# The ONE residual difference from the dedicated "England — 30k" sub-tab is the
# geography sampling, not the correlations: the sub-tab draws occupation from
# each LOCAL AUTHORITY's Census-2021 TS063 profile and derives region from a
# population-weighted place draw, whereas this preset samples occupation_soc and
# region from the flat NATIONAL configs. Both are census-representative on every
# marginal; they differ only in that local-authority-level geographic texture.
# For that finer geography, drive generation through build_uk_preset_skeletons()
# / PRESET_FULL_SAMPLERS below, which delegate straight to the England generator.
