...
def _apply_final_consistency_sweep(s: dict) -> dict:
    ...
    if _grade_now.startswith("AB"):
        ...
        if _edu_now in _EDU_NOQ_G and random.random() < 0.92:
            _deg_opts = (["Some high school", "High school diploma / GED",
                          "Vocational / Technical degree"]
                         if isinstance(_age_now, int) and _age_now < 21
                         else ["Some high school", "High school diploma / GED",
                               "Vocational / Technical degree", "Bachelor's degree",
                               "Master's degree"])
            _deg_wts = ([30, 40, 30] if isinstance(_age_now, int) and _age_now < 21
                        else [5, 10, 12, 45, 28])
            s["education_level"] = random.choices(_deg_opts, weights=_deg_wts, k=1)[0]
        elif _edu_now in _EDU_MID_G and random.random() < 0.45:
            if not (isinstance(_age_now, int) and _age_now < 21):
                s["education_level"] = random.choices(
                    ["Bachelor's degree", "Master's degree",
                     "Doctoral degree (PhD, MD, JD, etc.)"],
                    weights=[55, 35, 10], k=1)[0]
