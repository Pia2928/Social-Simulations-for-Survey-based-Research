...
_FAITH_CENTRES = {
    "Muslim/Islam": 0.86, "Hindu": 0.70, "Sikh": 0.72,
    "Jewish": 0.55, "Buddhist": 0.45,
    "Christian: Baptist": 0.62, "Christian: Other Christian": 0.55,
    "Christian: Methodist": 0.52, "Christian: Roman Catholic": 0.46,
    "Christian: Church of England/Anglican": 0.34,
    "Christian (no specific denomination)": 0.34,
    "No religion": 0.10, "Prefer not to say": 0.22,
}
...
def _apply_final_consistency_sweep(s: dict) -> dict:
    ...
    # ── Life satisfaction ↔ health (hard guarantee) ────────────────────────
    # A person with "Very bad" or "Bad" self-rated health should not score
    # 9+ on life satisfaction — the two measures are strongly correlated
    # empirically (r ≈ 0.35 in WVS). We also cap the optimism ceiling for
    # "Fair" health, since the Damian case (Fair health, life-sat 9.33)
    # shows the two can diverge implausibly.
    _hl = s.get("general_health")
    _ls = s.get("wvs_life_satisfaction")
    if isinstance(_ls, (int, float)) and _hl:
        if _hl == "Very bad" and _ls > 5.0:
            s["wvs_life_satisfaction"] = round(random.uniform(1.0, 4.5), 2)
        elif _hl == "Bad" and _ls > 6.5:
            s["wvs_life_satisfaction"] = round(random.uniform(2.0, 5.5), 2)
        elif _hl == "Fair" and _ls > 8.0:
            s["wvs_life_satisfaction"] = round(random.uniform(4.0, 7.5), 2)
    ...
    # ── Happiness ↔ life-satisfaction final backstop ────────────────────────
    # Runs LAST so it overrides any compression from the health cap above.
    # "Very happy" with life-sat < 4 is a flat contradiction; fix it here.
    _ls_f = s.get("wvs_life_satisfaction")
    _hap_f = s.get("wvs_happiness")
    if isinstance(_ls_f, (int, float)) and _hap_f:
        if _hap_f == "Very happy" and _ls_f < 4.0:
            s["wvs_life_satisfaction"] = round(random.uniform(6.0, 9.0), 2)
        elif _hap_f == "Not at all happy" and _ls_f > 7.0:
            s["wvs_life_satisfaction"] = round(random.uniform(1.0, 4.0), 2)
