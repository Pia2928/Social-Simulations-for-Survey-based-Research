...
def _apply_age_correlations(s: dict) -> dict:
    ...
    # read as retirement, so only continued part-time/self-employment is kept.
        ...
        if ea not in ("Employee - part-time", "Self-employed"):
            p_ret = 0.80 if age < 70 else 0.90
    ...
    # follow UK LFS economic-activity-by-age: ~10% at 55-59, ~30% at 60-64.
                                       ...
                                       "Economically inactive - long-term sick"):
        p_ret = 0.06 + (age - 55) * 0.035   # 55→0.06 ... 65→0.41
        if random.random() < p_ret:
            s["economic_activity"] = "Retired"
            s["occupation_soc"] = "N/A"
        elif ea == "Retired":
...
def _apply_health_activity_correlations(s: dict) -> dict:
    ...
    elif disab == _DISABLED_LOT and health == "Good" and random.random() < 0.6:
    ...
    if health in _BAD_HEALTH and disab == "Not disabled" and random.random() < 0.6:
    ...
    if ea == "Economically inactive - long-term sick":
        if s.get("general_health") in ("Very good", "Good") and random.random() < 0.8:
            s["general_health"] = random.choices(
                ["Fair", "Bad", "Very bad"], weights=[35, 45, 20], k=1)[0]
        if s.get("disability") == "Not disabled" and random.random() < 0.8:
            s["disability"] = random.choices(
                [_DISABLED_LOT, _DISABLED_LITTLE], weights=[60, 40], k=1)[0]
    ...
    if isinstance(age, int) and 25 <= age < 66 and s.get("economic_activity") in _EMPLOYED:
        ...
        if badly and random.random() < 0.45:
            s["economic_activity"] = random.choices(
                ["Economically inactive - long-term sick",
                 "Employee - part-time",
                 "Economically inactive - other"],
    ...
    if isinstance(age, int) and age < 65 \
            ...
            and s.get("economic_activity") not in (
                "Economically inactive - long-term sick",):
        if s.get("general_health") == "Good" and random.random() < 0.35:
            s["general_health"] = "Very good"
        elif s.get("general_health") == "Fair" and age < 45 \
...
_RELIGION_BY_ETHNICITY = {
    ...
    "White: English, Welsh, Scottish, Northern Irish or British": {
        _NO_REL: 43, _CHR_ANGLICAN: 25, _CHR_NONE_SPEC: 14, _CHR_CATHOLIC: 9,
        "Christian: Methodist": 2, "Christian: Baptist": 1,
        _CHR_OTHER: 2, "Prefer not to say": 4},
}
...
def _apply_attitude_correlations(s: dict) -> dict:
    ...
    # ── Education (+age) → libertarian-authoritarian ────────────────────────
    la = s.get("scale_lib_auth")
    if isinstance(la, (int, float)):
        offset = 0.0
        if edu in _EDU_HIGH:
            offset -= random.uniform(0.4, 0.9)   # graduates → more libertarian
        elif edu in _EDU_LOW:
            offset += random.uniform(0.2, 0.6)   # low ed → more authoritarian
    ...
    if isinstance(lr, (int, float)) and random.random() < 0.5:
        ...
        if grade.startswith("AB") or income in _HIGH_INCOME:
            offset += random.uniform(0.1, 0.4)   # slightly more right/free-market
        elif grade.startswith("DE") or income in _LOW_INCOME:
            offset -= random.uniform(0.1, 0.3)   # slightly more left
        if offset:
...
def _apply_wvs_correlations(s: dict) -> dict:
    ...
    # attitude correlations in the WVS UK data.
    if high_relig:
        # Stronger pull for very religious (God >= 9 or self-ID religious+high God)
        very_relig = (isinstance(god, (int, float)) and god >= 9)
        strong = (2.0, 4.0) if very_relig else (1.0, 3.0)
        mild = (0.5, 2.0) if very_relig else (0.5, 1.5)
        for k, span, prob in (
            ("wvs_just_homosexuality", strong, 0.6),
            ("wvs_just_abortion",      strong, 0.6),
            ("wvs_just_euthanasia",    strong, 0.55),
            ("wvs_just_sex_before_marriage", strong, 0.5),
            ("wvs_just_divorce",       mild,   0.4),
        ):
            v = s.get(k)
            if isinstance(v, (int, float)) and random.random() < prob:
                s[k] = round(max(1.0, v - random.uniform(*span)), 2)
    ...
    # ── Generalised trust ↔ openness to immigration / out-groups ────────────
    # Social-capital and contact research links generalised trust to out-group
    # acceptance. A modest association: trusting people lean toward accepting
    # immigrant/other-race neighbours and more open immigration policy; low
    # trust leans the other way. Kept weak (probabilistic, partial) to match the
    # modest effect sizes in the literature.
    trust = s.get("wvs_generalised_trust")
    trust_nat = s.get("wvs_trust_other_nationality")
    open_person = (trust == "Most people can be trusted") or \
                  (trust_nat in ("Trust completely", "Trust somewhat"))
    closed_person = (trust == "Need to be very careful") and \
                    (trust_nat in ("Do not trust very much", "Do not trust at all"))

    if closed_person:
        # Low trust → a bit more likely to not want immigrant/other-race
        # neighbours and to favour immigration limits.
        if s.get("wvs_neighbour_immigrants") == "Not mentioned" and random.random() < 0.10:
            s["wvs_neighbour_immigrants"] = "Mentioned"
        if s.get("wvs_immigration_policy") == "Let anyone come who wants to" \
                and random.random() < 0.5:
            s["wvs_immigration_policy"] = random.choices(
                ["Let people come as long as jobs available",
                 "Place strict limits on number of foreigners"],
                weights=[45, 55], k=1)[0]
        elif s.get("wvs_immigration_policy") == "Let people come as long as jobs available" \
                and random.random() < 0.25:
            s["wvs_immigration_policy"] = "Place strict limits on number of foreigners"
    elif open_person:
        # High trust → less likely to object to immigrant/other-race neighbours
        # and more open on policy.
        if s.get("wvs_neighbour_immigrants") == "Mentioned" and random.random() < 0.5:
            s["wvs_neighbour_immigrants"] = "Not mentioned"
        if s.get("wvs_neighbour_diff_race") == "Mentioned" and random.random() < 0.5:
            s["wvs_neighbour_diff_race"] = "Not mentioned"
        if s.get("wvs_immigration_policy") == "Prohibit people coming from other countries" \
                and random.random() < 0.6:
            s["wvs_immigration_policy"] = random.choices(
                ["Place strict limits on number of foreigners",
                 "Let people come as long as jobs available"],
                weights=[40, 60], k=1)[0]
        elif s.get("wvs_immigration_policy") == "Place strict limits on number of foreigners" \
                and random.random() < 0.20:
            s["wvs_immigration_policy"] = "Let people come as long as jobs available"
    ...
    # ── Attitudes toward homosexuality: justifiability ↔ same-sex parenting ──
    # These two items measure the same underlying attitude and should move
    # together. Someone who rates homosexuality near "never justifiable" should
    # not also strongly agree same-sex couples make good parents, and vice
    # versa. Reconcile the clear clashes in both directions, keeping the middle
    # ground (mild views can legitimately pair with a neutral parenting stance).
    ...
    if isinstance(just_h, (int, float)) and ss_par is not None:
        # Disapproving of homosexuality (<=3) but pro same-sex parenting → pull
        # the parenting view down to match.
        if just_h <= 3 and ss_par in ("Agree strongly", "Agree"):
            s["wvs_homosexual_parents"] = random.choices(
                ["Disagree", "Disagree strongly", "Neither agree nor disagree"],
                weights=[45, 35, 20], k=1)[0]
        # Accepting of homosexuality (>=8) but anti same-sex parenting → pull the
        # parenting view up.
        elif just_h >= 8 and ss_par in ("Disagree", "Disagree strongly"):
            s["wvs_homosexual_parents"] = random.choices(
                ["Agree", "Agree strongly", "Neither agree nor disagree"],
                weights=[45, 35, 20], k=1)[0]
        # Mid-low justifiability (3-5) shouldn't pair with "agree strongly".
        elif just_h < 5 and ss_par == "Agree strongly":
            s["wvs_homosexual_parents"] = random.choices(
...
def _apply_value_coherence(s: dict) -> dict:
    ...
    removed.
    """
    # ── 1. lib_auth  <->  WVS social/moral items ───────────────────────────
    # scale: 1 = social libertarian .. 5 = social authoritarian
    la = s.get("scale_lib_auth")
    if isinstance(la, (int, float)):
        sig = []
        jh = s.get("wvs_just_homosexuality")
        if isinstance(jh, (int, float)):
            sig.append((jh - 1) / 9.0)                 # 0 = illiberal, 1 = liberal
        ssp = s.get("wvs_homosexual_parents")
        if ssp is not None:
            sig.append({"Agree strongly": 1.0, "Agree": 0.78,
                        "Neither agree nor disagree": 0.5,
                        "Disagree": 0.22, "Disagree strongly": 0.0}.get(ssp, 0.5))
        mbl = s.get("wvs_gender_men_leaders")
        if mbl is not None:
            sig.append({"Strongly disagree": 1.0, "Disagree": 0.75,
                        "Agree": 0.25, "Agree strongly": 0.0}.get(mbl, 0.5))
        imm = s.get("wvs_immigration_policy")
        if imm is not None:
            sig.append({"Let anyone come who wants to": 1.0,
                        "Let people come as long as jobs available": 0.68,
                        "Place strict limits on number of foreigners": 0.28,
                        "Prohibit people coming from other countries": 0.0
                        }.get(imm, 0.5))
        if sig:
            libness = sum(sig) / len(sig)
            _POP_MEAN = 0.72          # this population's mean social liberalism
            target = 3.35 - 3.6 * (libness - _POP_MEAN)
            new = 0.30 * la + 0.70 * target + random.uniform(-0.25, 0.25)
            new = min(5.0, max(1.0, new))
            # Hard guards on the flat contradictions at the extremes.
            if libness >= 0.85:
                new = min(new, 3.2)   # uniformly liberal cannot be authoritarian
            elif libness <= 0.30:
                new = max(new, 3.5)   # uniformly illiberal cannot be libertarian
            s["scale_lib_auth"] = round(new, 2)

    # ── 2. left_right  <->  WVS economic items ─────────────────────────────
    ...
    # ── 3. welfarism  <->  left_right + the redistribution items ───────────
    ...
    # ── 4. The WVS ethical items load on ONE permissiveness dimension ──────
    ...
    # (abortion really is the most contested item, and stays freer).
    _eth = ["wvs_just_homosexuality", "wvs_just_divorce",
            "wvs_just_abortion", "wvs_just_sex_before_marriage"]
    vals = [s[k] for k in _eth if isinstance(s.get(k), (int, float))]
    if len(vals) >= 2:
        latent = sum(vals) / len(vals)
        for k in _eth:
            v = s.get(k)
            if isinstance(v, (int, float)):
                nv = 0.40 * v + 0.60 * latent + random.uniform(-0.7, 0.7)
                # Blending toward a mean compresses the scale; nudge outward so
                # each item keeps its survey-level population mean.
                nv += 0.55 if nv > 5.0 else -0.25
                s[k] = round(min(10.0, max(1.0, nv)), 2)
        ab = s.get("wvs_just_abortion")
        if isinstance(ab, (int, float)) and random.random() < 0.3:
            s["wvs_just_abortion"] = round(
                min(10.0, max(1.0, ab - random.uniform(0.5, 2.0))), 2)

    # Strong disapproval of homosexuality cannot coexist with support for
    # same-sex parenting (and vice versa).
    jh = s.get("wvs_just_homosexuality")
    ssp = s.get("wvs_homosexual_parents")
    if isinstance(jh, (int, float)) and ssp is not None:
        if jh <= 3 and ssp in ("Agree strongly", "Agree"):
            s["wvs_homosexual_parents"] = random.choices(
                ["Disagree", "Disagree strongly", "Neither agree nor disagree"],
                weights=[45, 35, 20], k=1)[0]
        elif jh >= 8 and ssp in ("Disagree", "Disagree strongly"):
            s["wvs_homosexual_parents"] = random.choices(
                ["Agree", "Agree strongly", "Neither agree nor disagree"],
                weights=[45, 35, 20], k=1)[0]

    # ── 5. RELIGION: the structural field and the WVS religiosity items ────
...
def _apply_final_consistency_sweep(s: dict) -> dict:
    ...
    # ── Homosexuality justifiability vs same-sex parenting (hard guarantee) ──
    just_h = s.get("wvs_just_homosexuality")
    ss_par = s.get("wvs_homosexual_parents")
    if isinstance(just_h, (int, float)) and ss_par is not None:
        if just_h <= 3 and ss_par in ("Agree strongly", "Agree"):
