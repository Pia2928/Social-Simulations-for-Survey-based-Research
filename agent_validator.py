...
def check_health(s) -> list:
    """Health ↔ Disability ↔ Economic activity ↔ Life satisfaction."""
    v = []
    ...
    # Life satisfaction should not be implausibly high for poor/fair health.
    ls = s.get("wvs_life_satisfaction")
    if isinstance(ls, (int, float)):
        if health == "Very bad" and ls > 5.0:
            v.append(Violation("health", "hard",
                f"'very bad' health but life satisfaction {ls}/10",
                ("general_health", "wvs_life_satisfaction")))
        elif health == "Bad" and ls > 6.5:
            v.append(Violation("health", "soft",
                f"'bad' health but life satisfaction {ls}/10",
                ("general_health", "wvs_life_satisfaction")))
        elif health == "Fair" and ls > 8.0:
            v.append(Violation("health", "soft",
                f"'fair' health but life satisfaction {ls}/10",
                ("general_health", "wvs_life_satisfaction")))
    return v
...
def check_value_coherence(s) -> list:
    """
    The BSA economic scale must agree with the WVS economic items, and
    religion must be internally coherent with the WVS religiosity items.
    (The social/moral fields have been removed from the schema.)
    """
    v = []

    # left_right (1=left .. 5=right) vs the economic answers.
    lr = s.get("scale_left_right")
    eq = s.get("wvs_econ_income_equality")
    po = s.get("wvs_econ_private_ownership")
...
def check_cross_field(s) -> list:
    """Catches the independent-sampling contradictions: education/student/grade,
    health/disability, happiness/life-sat, apolitical/participation."""
    v = []
    ea  = s.get("economic_activity", "")
    edu = s.get("education_level", "")
    health = s.get("general_health", "")
    ...
    if ea == "Full-time student" and edu in _NO_QUAL:
        v.append(Violation("cross_field", "hard",
            f"full-time student with education level '{edu}'",
            ("economic_activity", "education_level")))

    if ea == "Full-time student" and grade.startswith("AB"):
        v.append(Violation("cross_field", "hard",
            f"full-time student graded {grade} (students should be D/E)",
            ("economic_activity", "social_grade")))

    if health == "Very bad" and disab == "Not disabled":
        v.append(Violation("cross_field", "hard",
            "'very bad' health but not disabled",
            ("general_health", "disability")))

    # Religion-specific belief checks
    religion = s.get("religion", "") or ""
    _STRONG_FAITH = ("Roman Catholic", "Muslim", "Hindu", "Sikh", "Jewish")
    if any(f in religion for f in _STRONG_FAITH) and s.get("wvs_believe_in_god") == "No":
        v.append(Violation("cross_field", "soft",
            f"{religion} agent does not believe in God",
            ("religion", "wvs_believe_in_god")))

    # AB grade should not have no formal education or primary school
    if (s.get("social_grade") or "").startswith("AB")             and s.get("education_level") in ("No formal education", "Primary school"):
        v.append(Violation("cross_field", "soft",
            f"AB grade with education level '{s.get('education_level')}'",
            ("social_grade", "education_level")))

    lean = s.get("political_leaning")
    if lean == "Apolitical":
        if s.get("wvs_vote_national") == "Always":
            v.append(Violation("cross_field", "hard",
                "apolitical but always votes",
                ("political_leaning", "wvs_vote_national")))
        if s.get("wvs_action_demonstration") == "Have done":
            v.append(Violation("cross_field", "hard",
                "apolitical but has demonstrated",
                ("political_leaning", "wvs_action_demonstration")))
    ...
    if isinstance(ls, (int, float)) and hap:
        if hap == "Very happy" and ls < 5.0:
            v.append(Violation("cross_field", "hard",
                f"'very happy' but life satisfaction {ls}/10",
                ("wvs_happiness", "wvs_life_satisfaction")))
        elif hap == "Not at all happy" and ls > 6.0:
            v.append(Violation("cross_field", "hard",
                f"'not at all happy' but life satisfaction {ls}/10",
                ("wvs_happiness", "wvs_life_satisfaction")))
    return v
...
_ALL_CHECKS: list = [
    ...
    check_wellbeing, check_value_coherence, check_cross_field,
]
