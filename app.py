...
def load_agents(force_reload: bool = False) -> list:
    """Load agents with session-state caching so we only hit Supabase once
    per session rather than on every Streamlit re-run."""
    if force_reload or "_cached_agents" not in st.session_state:
        st.session_state["_cached_agents"] = load_all_agents()
    return st.session_state["_cached_agents"]
