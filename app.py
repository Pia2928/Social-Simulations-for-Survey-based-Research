...
def _sample_demographic_skeletons(n, field_configs):
    ...
    for _ in range(n):
        ...
        # Keep age consistent with employment/education/occupation when the
        # preset includes an employment status (the England presets do). This
        # mirrors the England sub-tab and prevents e.g. 18-year-olds coming out
        # as "Retired". Pure non-England presets (no economic_activity) are
        # left untouched.
...
{{
  ...
  "income_bracket": {{
    <use ONLY these exact strings: {income_str}>: <relative weight number>
  }},
            ...
            for i, qid in enumerate(list(st.session_state.sqe_q_ids)):
                with st.container():
                    ...
                    if q_type == "open_ended":
                        ...
                        with lc1:
                            st.selectbox("Scale", [3, 5, 7, 10], index=1, key=f"sqe_scale_{qid}")
                        with lc2:
                        ...
                        with gc1:
                            st.selectbox("Scale", [3, 4, 5, 7, 10], index=2, key=f"sqe_gscale_{qid}")
                        with gc2:
            ...
            with col_go:
                if st.button(
                    ...
                    for i, skeleton in enumerate(skeletons):
                        ...
                        progress_bar.progress((i + 1) / len(skeletons))
